...
rastilho_têr_linha_por_tipo(Rastilho_Tipo tipo) {
	...
	câmbio (tipo)
	{
	caso rastilho__carece_concedido: 				{ linha = "Carece concedido/clave, indefinido."; 									encerra; }
	caso rastilho__carece_concessão: 				{ linha = "Carece concessão/separador."; 											encerra; }
	caso rastilho__carece_concessão_válida: 		{ linha = "A concessão é inválida"; 												encerra; }
	caso rastilho__carece_ficha_válida: 			{ linha = "A ficha é inválida"; 													encerra; }
	caso rastilho__carece_valôr: 					{ linha = "Carece valôr à clave"; 													encerra; }
	caso rastilho__expressão_excedente: 			{ linha = "Existem mais operações do que o esperado, formatação incorrecta."; 		encerra; }
	caso rastilho__encerro_forçado: 				{ linha = "Encerrou-se o ficheiro forçadamente."; 									encerra; }
	caso rastilho__comentário: 						{ linha = "Expressão comentada."; 													encerra; }
...
operação_re_definir(int operação_n, Grade* expressão, Expectação expectação, Operação_Tipo operação_tipo, size_t linha_t) {
	...
	se (operações->índice iqual inválido) { abort(); }
	...
	linha_introduzir_charactére(linha_nil, 0, &void_como(operação_linha->elemento, Linha));
...
Grade
operação_falha() {
	devolve grade_falha(linhar_(estructura_instância(Operação)));
}
...
expressão_têr_por_ficha(Grade* expressões, Linha ficha) {
	...
	enquanto (expressão->índice iqual expressão_n) {
		DESBRAGA_MENSAGEM("%d - %d", expressão->índice, expressão_n);

		se (expressão_operação_têr_por_ficha(expressão, ficha).índice differente -1) {
			DESBRAGA_TÓPICO("%s", ficha);
...
#define operação_				(void_como(operação->elemento, Operação*))
	...
	Grade resultado = operação_falha();
  	...
  	enquanto (operação->índice iqual operação_n) {
		DESBRAGA_TÓPICO("%s %d - %d", operação->constatação, operação_->índice, operação_n);
		DESBRAGA_TÓPICO("Tipo - %d, %d", operação_->tipo, tipo);
...
Grade
expressão_falha() {
	devolve grade_falha(linhar_(estructura_instância(Expressão)));
}
...
#define ficha_valôr_linha_		void_como(valôrDaFicha_linha->elemento, Linha)
	...
	se (clave_têr_por_tipo(clave_ficha).pala[2] iqual ef->charactére) {
		...
		Grade* expressão_por_ficha = expressão_têr_por_ficha(expressões, ef->ficha);
		Grade valôrDaFicha = expressão_operação_têr_por_tipo(expressão_por_ficha, operação__valôr);

		DESBRAGA_TÓPICO("%d", valôrDaFicha.índice);
...
#define operação_linha_       	void_como(operação_linha->elemento, Linha)
	...
	se (operação_->expectação iqual expectação__concedido ou operação_->expectação iqual expectação__nil ou operação_->expectação iqual expectação__valôr)
	{
		Grade* operação_linha = grade_procurar(operação->filho, linhar_(estructura_instância(Operação).linha), índice__qualquer);
...
#define operação_linha_       	void_como(operação_linha->elemento, Linha)
	...
	se (clave_têr_por_tipo(clave_lêr).pala[ef_.clave_n] iqual ef_.charactére) {
		...
		ef->operação_aparar_e_re_definir(ef, intérprete, expectação__valôr, operação__concessão_directa);
		...
		linha_introduzir_charactére(ef->charactére, ef->operação_linha_n, &operação_linha_);
...
#define expressão_n_			(*ef_.expressão_n)
#define operação_				void_como(operação->elemento, Operação)
#define operação_linha_       	void_como(operação_linha, Linha)
	...
	se (
		clave_têr_por_tipo(clave_lêr).pala[ef_.clave_n] iqual ef_.pilha.conteúdo[ef_.recúo - 1]
		e clave_têr_por_tipo(clave_lêr).pala[ef_.clave_n] iqual ef_.pilha.conteúdo[ef_.recúo - 2]
	) {
		Grade* expressões = grade_procurar(intérprete->filho, linhar_(estructura_instância(Intérprete).expressão), índice__qualquer);
		Grade* expressão = grade_procurar(expressões->filho, linhar_(estructura_instância(Expressão)), expressão_n_);
		Grade* operações = grade_procurar(expressão, linhar_(estructura_instância(Expressão).operação), índice__qualquer);
		...
		ef->operação_aparar_e_re_definir(ef, intérprete, expectação__valôr, operação__concessão_passiva);
		
		ef_.operação_linha_n = 0;
		linha_introduzir_charactére(ef->pilha.conteúdo[ef->recúo - 1], ef->operação_linha_n, &operação_linha_); ef->operação_linha_n++;
		linha_introduzir_charactére(ef->pilha.conteúdo[ef->recúo - 2], ef->operação_linha_n, &operação_linha_);
...
#define operação_linha_       	(void_como(operação_linha->elemento, Linha))
	...
	se (expressão_operação_têr_por_tipo(expressão, operação__concessão_passiva).índice differente inválido) {
		Grade* operações = grade_procurar(expressão, linhar_(estructura_instância(Expressão).operação), índice__qualquer);
		Grade* operação = grade_procurar(operações->filho, linhar_(estructura_instância(Operação)), ef->operação_n);
		Grade* operação_linha = grade_procurar(operação->filho, linhar_(estructura_instância(Operação).linha), índice__qualquer);
		...
		*intérprete = *intérprete_agregado;

		expressões = grade_procurar(intérprete->filho, linhar_(estructura_instância(Intérprete).expressão), índice__qualquer);
		expressão = grade_procurar(expressões->filho, linhar_(estructura_instância(Expressão)), 0);
		operações = grade_procurar(expressão, linhar_(estructura_instância(Expressão).operação), índice__qualquer);
		...
		grade_des_allocar(&intérprete_agregado);
...
void interpretar_linha__operação_aparar_e_re_definir(EF_Interpretar* ef, Grade* intérprete, Expectação expectação, Operação_Tipo operação_tipo) {
#if defined(DEFINIÇÃO)
...
#define operação_linha_       	void_como(operação_linha->elemento, Linha)
	...
	ef->operação_n++;
	operação_re_definir(ef->operação_n, expressão, expectação, operação_tipo, sizeof(char) * 2);
...
#define recúo_                	(ef_.recúo - 1)
	...
	if (expressões iqual nil) { devolve; }

	Grade* _expressões = grade_procurar(intérprete->filho, linhar_(estructura_instância(Intérprete).expressão), índice__qualquer);
	Grade* expressão = grade_procurar(expressões->filho, linhar_(estructura_instância(Expressão)), expressão_n_);
	...
	while (expressões[expressão_n].índice igual expressão_n) {
		_expressões = memória_re_allocar((posição + 1) * sizeof(Grade), _expressões);
		_expressões[posição] = expressões[expressão_n];
		_expressões[posição].índice = posição;
...
Grade* intérprete_confeccionar(EF_Interpretar ref ef, Grade ref intérprete, Grade caminho) {
#if defined(DEFINIÇÃO)
...
#define recúo_                	(ef_.recúo - 1)
	...
	Grade* intérprete_confeccionado = memória_allocar(sizeof(Grade));

	intérprete_confeccionado[0] = grade_falha(linhar_(estructura_instância(Intérprete)));
	intérprete_confeccionado[0].tipo = lsve_tipo_intérprete;
	...
	Grade* intérprete_confeccionado_expressões = grade_procurar(intérprete_confeccionado->filho, linhar_(estructura_instância(Intérprete).expressão), índice__qualquer);
	...
	*grade_procurar(intérprete_confeccionado_expressões, linhar_(estructura_instância(Expressão)), 0) = grade_falha(linhar_(estructura_instância(Expressão)));

	intérprete_agregar(&ef_, intérprete_confeccionado, expressões->filho, 0);
	interpretar(&ficheiro_linhas, &(*intérprete_confeccionado));

	devolve intérprete_confeccionado;
...
void
interpretar_linha(int ref expressão_n, Grade ref linha, Grade ref intérprete) {
#if defined(DEFINIÇÃO)
...
#define recúo_                			(ef.recúo - 1)
    ...
    while(ef.recúo > 1) {
		...
		*/
		se (ef.charactére iqual linha_nil) continua;
		...
		/*
			...
			Reporta rastilho.
		*/
		se (ef.clave_verificar_encerro_forçado(&ef, intérprete)) {
		...
		*/
		...
		se (ef.clave_verificar_comentário(&ef, intérprete)) {
			encerra;
		}
		...
		se (operação_->expectação iqual expectação__ficha_fecha) {
			DESBRAGA_TÓPICO("FICHA - %s", ef.ficha);
		...
		clave_têr_por_tipo(clave_ficha).pala[2] iqual ef.charactére)
		{
			...
			{
				ef.intérprete_rastilho_definir(ef, intérprete, rastilho__carece_concessão_válida);
				expressão_n_++;
				encerra;
			}
		}
		...
		*/
		se (ef.charactére iqual linha_salta ou ef.charactére iqual EOF) {
			...
			se (ef.operação_n iqual 0 e operação_linha_[ef.operação_linha_n] differente linha_nil) {
				ef.intérprete_rastilho_definir(&ef, intérprete, rastilho__carece_concessão);
				abort();
			}
			ou_se (ef.operação_n iqual 0 e operação_linha_[ef.operação_linha_n] iqual linha_nil) {
				...
				*expressão = expressão_falha();
				ef.intérprete_rastilho_definir(&ef, intérprete, rastilho__nil);
				DESBRAGA_MENSAGEM("\n");
				abort();
			}

			//se (ef.operação_n iqual 1) {
			//	ef.intérprete_rastilho_definir(&ef, intérprete, rastilho__carece_valôr);
			//	abort();
			//}
			...
			// Todos as operações que precisam dos valôres completos são validados após o registro de toda a linha.
			//se (ef.expressão_verificar_concessão_corredora(&ef, intérprete)) continua;
			se (ef.expressão_verificar_concessão_passiva(&ef, intérprete)) continua;
			//se (ef.expressão_verificar_concessão_objectiva(&ef, intérprete)) continua;
			//se (ef.expressão_verificar_concessão_selectiva(&ef, intérprete)) continua;
		...
		*/
		se (operação_->expectação iqual expectação__concessão) {
			...
			se (ef.clave_verificar_concessão_passiva(&ef, intérprete)) {
				DESBRAGA_TÓPICO("CONCESSÃO PASSIVA");

				//se (ef.clave_verificar_concessão_selectiva(ef, intérprete)) {
				//	ef.operação_aparar_e_re_definir(ef, intérprete);
				//	continua;
				//}

				ef.operação_aparar_e_re_definir(&ef, intérprete, expectação__valôr, operação__valôr);
				operação = grade_procurar(operações->filho, linhar_(estructura_instância(Operação)), ef.operação_n);
				operação_linha = grade_procurar(operação->filho, linhar_(estructura_instância(Operação).linha), índice__qualquer);
				continua;
			}

			se (ef.clave_verificar_concessão_directa(&ef, intérprete)) {

				DESBRAGA_TÓPICO("CONCESSÃO DIRECTA");

				/*
				if (ef.clave_verificar_concessão_objectiva(ef, intérprete)) {
					DESBRAGA_TÓPICO("CONCESSÃO OBJECTIVA");

					continua;
				}

				if (ef.clave_verificar_concessão_corredora(ef, intérprete)) {
					ef.operação_aparar_e_re_definir(ef, intérprete);
					continua;
				}
				*/
...
#define linhas_                 (*linhas)
    ...
    enquanto (linhas_[expressão_n].índice iqual expressão_n) {
		...
		interpretar_linha(&expressão_n, &linhas_[expressão_n], intérprete);
